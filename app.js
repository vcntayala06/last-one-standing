...
const QUESTION_BANK_SOURCE={...window.LOS_QUESTION_BANK_DATA,bankVersion:"stage-6.36-visual-foundation",questions:[...window.LOS_QUESTION_BANK_DATA.questions,...window.LOS_QUESTION_BANK_BATCH_1.questions,...window.LOS_QUESTION_BANK_BATCH_2.questions,...window.LOS_QUESTION_BANK_BATCH_3.questions,...window.LOS_QUESTION_BANK_BATCH_4.questions,...window.LOS_QUESTION_BANK_BATCH_5.questions,...window.LOS_QUESTION_BANK_BATCH_6.questions,...window.LOS_QUESTION_BANK_BATCH_7.questions,...window.LOS_QUESTION_BANK_BATCH_8.questions,...window.LOS_QUESTION_BANK_BATCH_9.questions]};
...
const BUILD_INFO={stage:"6.36",version:"visual-foundation-home-whos-in-avatar",builtAt:"2026-08-27"},transitionDiagnostics=[],screenLifetimeDiagnostics=[],answerDiagnostics=[],playerUpDiagnostics=[],audioDiagnostics={tick:"off",buzzerFired:false},transitionDebugEnabled=new URLSearchParams(location.search).get("playtestDebug")==="1"||localStorage.getItem("los_playtest_debug")==="1";
...
function shell(title,content,footer=""){const mic=state.screen==="setup"?"":micToggleMarkup();return `<section class="screen"><div class="shell"><header class="topbar"><div></div><div class="topbar-title">${title||""}</div><div class="topbar-tools">${mic}</div></header><div class="content">${content}</div><footer class="footer">${footer}</footer></div></section>${state.voiceOn?`<div id="voiceDiagnostic" class="voice-diagnostic">MIC LISTENING</div>`:""}`}
...
function centralNavIntent(h,isFinal=false,confidence=0){
 ...
 if(state.screen==="packs"){
  ...
  if(/^(back|go back)$/.test(n)){if(!isFinal)return false;return voiceOnce("back:packs",()=>go("home","packs-back"))}
 }
...
function packs(focusGroup="",focusValue=""){
 ...
 const musicPanel=topics.has("entertainment")&&entertainment.has("music")?`<section class="music-filter-panel" aria-labelledby="musicFilterTitle"><div id="musicFilterTitle" class="pack-instruction">MUSIC MIX — CHOOSE ANY COMBINATION</div><div class="music-subcategory-grid"><button class="btn music-subcategory ${musicSelected.size?"":"selected"}" data-all-music aria-pressed="${!musicSelected.size}">ALL MUSIC</button>${musicButtons}</div></section>`:"";
 const secondary=`${topics.has("entertainment")?`<section class="hierarchy-panel"><div class="pack-instruction">ENTERTAINMENT</div><div class="pack-grid secondary-grid">${optionButtons(ENTERTAINMENT_OPTIONS,entertainment,"data-entertainment")}</div>${musicPanel}</section>`:""}${topics.has("transit")?`<section class="hierarchy-panel"><div class="pack-instruction">TRANSIT</div><div class="pack-grid secondary-grid">${optionButtons(TRANSIT_OPTIONS,transit,"data-transit")}</div></section>`:""}${topics.has("brain")?`<section class="hierarchy-panel"><div class="pack-instruction">BRAIN GAMES</div><div class="pack-grid secondary-grid">${optionButtons(BRAIN_OPTIONS,brain,"data-brain")}</div></section>`:""}`;
 app.innerHTML=shell("CHOOSE YOUR GAME",`<div class="pack-picker"><div id="packError" class="setup-error" role="alert" aria-live="assertive"></div><section class="hierarchy-panel"><div class="pack-instruction">AUDIENCE / GAME ENVIRONMENT</div><div class="pack-grid audience-grid">${optionButtons(AUDIENCE_OPTIONS,audiences,"data-audience")}</div></section><section class="hierarchy-panel"><div class="pack-instruction">TOPICS / VIBES — MIX ANY</div><div class="pack-grid topic-grid">${optionButtons(TOPIC_OPTIONS,topics,"data-topic")}</div><div class="pack-summary">${topics.size?"SELECTED TOPICS ACTIVE":"NO TOPIC SELECTED — GENERAL MIX"}</div></section>${secondary}</div>`,`<button id="back" class="btn">BACK</button><button id="continuePacks" class="btn primary large" data-voice="PLAY">PLAY</button>`);
 document.querySelectorAll("[data-audience]").forEach(b=>b.onclick=()=>setAudience(b.dataset.audience));document.querySelectorAll("[data-topic]").forEach(b=>b.onclick=()=>toggleTopic(b.dataset.topic));for(const group of ["entertainment","transit","brain"])document.querySelectorAll(`[data-${group}]`).forEach(b=>b.onclick=()=>toggleHierarchySubcategory(group,b.dataset[group]));document.querySelectorAll("[data-music-subcategory]").forEach(button=>button.onclick=()=>toggleMusicSubcategory(button.dataset.musicSubcategory));document.querySelector("[data-all-music]")?.addEventListener("click",selectAllMusic);document.getElementById("back").onclick=()=>go("home","packs-back");document.getElementById("continuePacks").onclick=continueFromPacks;startVoice("packs");if(focusGroup&&focusValue)document.querySelector(`[data-${focusGroup}="${focusValue}"]`)?.focus()
}
...
function selectSavedPlayerByVoice(raw){
 ...
 if(state.players.some(p=>p.id===player.id)){voiceFeedback(`✓ ${playerDisplayName(player).toUpperCase()} SELECTED`,"action");return true}
 if(state.mode==="solo")state.players=[player];else state.players.push(player);state.selectedIds=state.players.map(p=>p.id);voiceFeedback(`✓ ${playerDisplayName(player).toUpperCase()}`,"action");players();return true
}
function openSavedPlayerPicker(){
 ...
 const close=()=>{overlay.remove();players();document.getElementById("selectPlayers")?.focus()};
 const draw=()=>{const profiles=savedProfiles(),selected=new Set(state.players.map(p=>p.id)),filtered=profiles.filter(p=>playerSearchText(p).includes(query.toLocaleLowerCase()));const rows=filtered.map(p=>{const chosen=selected.has(p.id),nickname=p.nickname&&nameKey(p.nickname)!==nameKey(p.name)?`<small>“${esc(p.nickname)}”</small>`:"";return `<article class="los-saved-player-row ${chosen?"selected":""}" data-saved-profile="${p.id}"><button type="button" class="los-saved-player-toggle" data-picker-player="${p.id}" aria-pressed="${chosen}" data-voice="${esc(playerDisplayName(p))}">${avatarArt(p.avatar)}<span><strong>${esc(playerDisplayName(p))}</strong>${nickname}<em>${chosen?"SELECTED FOR THIS GAME":"SELECT PLAYER"}</em></span><b aria-hidden="true">${chosen?"✓":"+"}</b></button><div class="los-saved-player-actions"><button type="button" class="los-mini-action" data-picker-avatar="${p.id}">AVATAR</button><button type="button" class="los-mini-action" data-picker-edit="${p.id}">EDIT</button></div></article>`}).join("");overlay.innerHTML=`<section class="los-sheet los-player-picker"><header><div><div class="los-sheet-kicker">SAVED PROFILES</div><h2>SELECT PLAYER</h2><p><strong>${selected.size}</strong> selected for this game</p></div><button type="button" class="los-close" data-picker-close aria-label="Close player picker">×</button></header><label class="los-player-search"><span>SEARCH PLAYERS</span><input type="search" value="${esc(query)}" autocomplete="off" placeholder="Name or nickname" aria-label="Search players"></label><div class="los-saved-player-list" aria-live="polite">${rows||`<div class="los-picker-empty"><strong>${profiles.length?"NO MATCHES":"NO SAVED PLAYERS YET"}</strong><span>${profiles.length?"Try another name.":"Close this picker and choose Add Player."}</span></div>`}</div><button type="button" class="los-action los-action-primary los-picker-done" data-picker-done>DONE</button></section>`;
  ...
  overlay.querySelectorAll("[data-picker-player]").forEach(button=>button.onclick=()=>{const p=profiles.find(x=>x.id===button.dataset.pickerPlayer);if(!p)return;if(selected.has(p.id))state.players=state.players.filter(x=>x.id!==p.id);else if(state.mode==="solo")state.players=[p];else state.players.push(p);state.selectedIds=state.players.map(x=>x.id);draw()});
 ...
 overlay.addEventListener("keydown",event=>{if(event.key==="Escape")close()});document.body.appendChild(overlay);draw();overlay.querySelector("input[type=search]")?.focus()
}
...
function players(){
 ensurePlayers();const selected=state.players.map(p=>normalizePlayer(p)),chips=selected.map(p=>`<article class="los-selected-player" data-selected-player="${p.id}">${avatarArt(p.avatar)}<strong>${esc(playerDisplayName(p))}</strong><button type="button" data-remove-player="${p.id}" aria-label="Remove ${esc(playerDisplayName(p))} from this game">×</button></article>`).join("");
 app.innerHTML=shell("",`<section class="los-roster"><div class="los-roster-heading"><div class="los-sheet-kicker">GAME ROSTER</div><h1>WHO’S IN?</h1><p>SELECTED FOR THIS GAME</p></div><div class="los-selected-players">${chips||`<div class="los-selected-empty"><strong>NO PLAYERS SELECTED</strong><span>Choose saved players or add someone new.</span></div>`}</div><div class="los-roster-actions"><button id="selectPlayers" class="los-action los-action-secondary" data-voice="SELECT PLAYER" data-voice-aliases="choose player|saved players">SELECT PLAYER</button><button id="add" class="los-action los-action-primary los-add-player">+ ADD PLAYER</button></div><div id="rosterError" class="roster-error" role="alert"></div><div class="subtle center">Say a saved player’s name, or say “Continue.”</div></section>`,`<button id="back" class="btn">BACK</button><button class="btn setup-exit-bottom" data-setup-exit data-voice="EXIT" data-voice-aliases="exit game|leave game|cancel game|quit setup|go home|back to home">EXIT</button><button id="continue" class="btn primary" data-voice="CONTINUE" data-voice-aliases="play|lets go|let s go|lets do it|let s do it|go|start|start game">CONTINUE</button>`);document.querySelector(".screen")?.classList.add("los-roster-screen");
 document.querySelectorAll("[data-remove-player]").forEach(button=>button.onclick=()=>{state.players=state.players.filter(p=>p.id!==button.dataset.removePlayer);state.selectedIds=state.players.map(p=>p.id);players()});
