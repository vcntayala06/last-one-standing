...
const TRANSIT_OPTIONS=[["transit","GENERAL TRANSIT"],["paratransit","PARATRANSIT"],["sunline","SUNLINE"]];
...
const QUESTION_BANK_SOURCE={...window.LOS_QUESTION_BANK_DATA,bankVersion:"stage-6.31-disney-expansion",questions:[...window.LOS_QUESTION_BANK_DATA.questions,...window.LOS_QUESTION_BANK_BATCH_1.questions,...window.LOS_QUESTION_BANK_BATCH_2.questions,...window.LOS_QUESTION_BANK_BATCH_3.questions,...window.LOS_QUESTION_BANK_BATCH_4.questions,...window.LOS_QUESTION_BANK_BATCH_5.questions,...window.LOS_QUESTION_BANK_BATCH_6.questions]};
...
const BUILD_INFO={stage:"6.31",version:"large-verified-disney-expansion",builtAt:"2026-08-25"},transitionDiagnostics=[],screenLifetimeDiagnostics=[],answerDiagnostics=[],playerUpDiagnostics=[],audioDiagnostics={tick:"off",buzzerFired:false},transitionDebugEnabled=new URLSearchParams(location.search).get("playtestDebug")==="1"||localStorage.getItem("los_playtest_debug")==="1";
...
function centralNavIntent(h,isFinal=false,confidence=0){
 ...
 if(state.screen==="packs"){
  if(/^(continue|start game)$/.test(n))return voiceOnce("continue:packs",()=>continueFromPacks());
...
function hierarchyFromLegacy(source={}){const packs=new Set(source.contentPacks||[]),topics=Array.isArray(source.topics)?source.topics:[],entertainment=Array.isArray(source.entertainmentSubcategories)?source.entertainmentSubcategories:[],transit=Array.isArray(source.transitSubcategories)?source.transitSubcategories:[],brain=Array.isArray(source.brainSubcategories)?source.brainSubcategories:[];return{audience:["general","work","kids"].includes(source.audience)?source.audience:packs.has("work")?"work":packs.has("kids")?"kids":"general",topics:[...new Set([...topics,...(packs.has("street")?["culture"]:[]),...(packs.has("transit")||packs.has("sunline")||packs.has("paratransit")?["transit"]:[]),...(packs.has("movies")||packs.has("music")||packs.has("disney")?["entertainment"]:[]),...(packs.has("known")||packs.has("riddles")?["brain"]:[])])],entertainmentSubcategories:[...new Set([...entertainment,...["movies","music","disney"].filter(x=>packs.has(x))])],transitSubcategories:[...new Set([...transit,...["transit","paratransit","sunline"].filter(x=>packs.has(x))])],brainSubcategories:[...new Set([...brain,...["known","riddles"].filter(x=>packs.has(x))])]}}
...
function back(){
 const m={setup:"home",packs:"setup",mode:"home",industry:"mode",difficulty:"mode",fun:"difficulty",players:"packs",time:state.mode==="solo"?"packs":"players",ready:state.mode==="solo"?"packs":"players"};
...
function playerUpCountdown(playerId,opening=false){
 ...
 const playerUpMessage=document.getElementById("playerUpMessage");renderDiagnostic.domText=playerUpMessage?.innerText||playerUpMessage?.textContent||"";bindGamebar();startVoice("handoff");const event=state.mode==="solo"?"soloTurn":opening?"firstTurn":"turn";hostSystem?.emit(event,{name:p.name,mode:state.mode});
