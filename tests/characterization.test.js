...
test("setup voice routes Original, Work Edition, and Solo",()=>{
  const cases=[["original","original","difficulty",false],["work edition","work","difficulty",false],["solo","solo","difficulty",false]];
...
test("Stage 2 navigation bypasses Extra Categories and Solo bypasses Who's In",()=>{
  for(const mode of ["original","work"]){const h=createHarness();try{const s=h.api.getState();h.api.mode();h.click(`[data-mode="${mode}"]`);h.timers.advance(220);assert.equal(s.screen,"difficulty");h.click("#cont");h.timers.advance(220);assert.equal(s.screen,"players");assert.notEqual(s.screen,"fun")}finally{h.close()}}
  const h=createHarness();try{const s=h.api.getState();s.players=[];h.api.mode();h.click('[data-mode="solo"]');h.timers.advance(220);assert.equal(s.screen,"difficulty");h.click("#cont");assert.equal(s.screen,"time");assert.equal(s.players.length,1);assert.ok(s.players[0].name.trim())}finally{h.close()}
});
...
test("difficulty selection and Continue work by voice",withHarness(h=>{const s=h.api.getState();s.mode="work";h.api.go("difficulty");h.timers.advance(220);h.speak("hard");assert.equal(s.difficulty,"hard");h.speak("continue");assert.equal(s.screen,"players")}));

test("Back follows the setup transition map",()=>{
  const cases=[["mode","home"],["industry","mode"],["fun","difficulty"],["players","setup"],["time","players"],["ready","players"]];
...
test("Continue advances Game Setup through Who's In to Showtime",withHarness(h=>{const s=h.api.getState();setValidPlayers(s);s.screen="setup";h.api.setup();h.speak("continue");assert.equal(s.screen,"players");h.timers.advance(220);h.speak("continue");assert.equal(s.screen,"ready");h.timers.advance(30000);assert.equal(s.screen,"ready");h.click("#showtimeStart");assert.equal(s.screen,"handoff");assert.ok(s.game)}));
...
test("player data mutations wait for final speech while Continue can use interim speech",withHarness(h=>{const s=h.api.getState();setValidPlayers(s);s.screen="players";h.api.players();h.speak("add player Casey",{final:false});assert.deepEqual(Array.from(s.players,p=>p.name),["Alex","Blair"]);h.speak("add player Casey",{final:true});assert.deepEqual(Array.from(s.players,p=>p.name),["Alex","Blair","Casey"]);h.speak("continue",{final:false});assert.equal(s.screen,"ready")}));

test("recognition alternatives route the first actionable transcript",withHarness(h=>{const s=h.api.getState();h.api.go("mode");h.timers.advance(220);h.speak("unrelated noise",{alternatives:["work edition"]});assert.equal(s.mode,"work");assert.equal(s.screen,"difficulty")}));
...
test("stale callbacks from an old recognition generation are ignored",withHarness(h=>{const s=h.api.getState(),old=h.recognition();h.api.go("mode");h.timers.advance(220);old.end();h.timers.advance(25);const current=h.recognition(),count=FakeSpeechRecognition.instances.length;assert.notEqual(current,old);old.emit("original");old.error("not-allowed");old.end();h.timers.advance(1000);assert.equal(s.screen,"mode");assert.equal(h.recognition(),current);assert.equal(FakeSpeechRecognition.instances.length,count);h.speak("work edition");assert.equal(s.screen,"difficulty")}));
...
test("a handled interim and final result cannot operate two setup screens",withHarness(h=>{const s=h.api.getState();s.screen="difficulty";h.api.difficulty();const r=h.recognition();r.emit("continue",{final:false,confidence:.01});assert.equal(s.screen,"players");h.timers.advance(220);r.emit("continue",{final:true,confidence:.01});assert.equal(s.screen,"players");const rejected=h.api.getVoiceDiagnostics().find(x=>x.reason==="final-after-handled-interim");assert.ok(rejected)}));
test("a voice navigation command arriving in the new locked context executes once after unlock",withHarness(h=>{const s=h.api.getState();h.api.go("difficulty");const r=h.recognition();r.emit("continue",{final:false,confidence:.01});r.emit("continue",{final:true,confidence:.01});assert.equal(s.screen,"difficulty");h.timers.advance(220);assert.equal(s.screen,"players");h.timers.advance(220);assert.equal(s.screen,"players")}));
test("Resume interim and final cannot resume and then answer the question",withHarness(h=>{const s=activeQuestion(h);s.game.current={q:"What planet?",a:"Mars"};s.game.answered=false;h.api.pauseGame();const r=h.recognition();r.emit("resume",{final:false,confidence:.01});assert.equal(s.screen,"question");r.emit("resume",{final:true,confidence:.01});assert.equal(s.screen,"question");assert.equal(s.game.answered,false);assert.equal(s.game.players[0].wrong,0)}));
...
test("exact setup controls remain interim-responsive while fuzzy wrappers remain final-only",withHarness(h=>{const s=h.api.getState();h.api.go("mode");h.timers.advance(220);h.speak("choose work edition",{final:false});assert.equal(s.screen,"mode");assert.equal(s.mode,"original");h.speak("work edition",{final:false});assert.equal(s.screen,"difficulty")}));

test("fuzzy visible-control matching activates only on final speech",withHarness(h=>{const s=h.api.getState();h.api.go("mode");h.timers.advance(220);h.speak("choose work edition",{final:false});assert.equal(s.screen,"mode");h.speak("choose work edition",{final:true});assert.equal(s.mode,"work");assert.equal(s.screen,"difficulty")}));
