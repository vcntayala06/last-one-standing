...
test("navigation interim waits and its final result operates exactly one setup screen",withHarness(h=>{const s=h.api.getState();s.screen="difficulty";h.api.difficulty();const r=h.recognition();r.emit("continue",{final:false,confidence:.01});assert.equal(s.screen,"difficulty");r.emit("continue",{final:true,confidence:.01});assert.equal(s.screen,"players");h.timers.advance(220);assert.equal(s.screen,"players")}));
