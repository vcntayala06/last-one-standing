...
test("Build 6.29 reveals multi-select Music filters only while Music is active",withHarness(h=>{const state=h.api.getState();state.screen="packs";state.contentPacks=["original"];state.musicSubcategories=[];h.api.packs();assert.equal(h.document.querySelector("[data-music-subcategory]"),null);h.click('[data-content-pack="music"]');for(const id of ["hip-hop","r-and-b","pop","rock","classic-rock","rock-and-roll","funk","soul","motown","lowrider-oldies","latin-oldies","regional-mexican","today","1970s","1980s","1990s","2000s"])assert.ok(h.document.querySelector(`[data-music-subcategory="${id}"]`),id);h.click('[data-music-subcategory="lowrider-oldies"]');h.click('[data-music-subcategory="today"]');assert.deepEqual(Array.from(state.musicSubcategories),["lowrider-oldies","today"]);assert.equal(h.document.querySelector('[data-all-music]').getAttribute("aria-pressed"),"false");h.click('[data-all-music]');assert.deepEqual(Array.from(state.musicSubcategories),[]);h.click('[data-content-pack="music"]');assert.equal(h.document.querySelector("[data-music-subcategory]"),null)}));
