...
test("Build 6.30 exposes Transit and Brain secondary choices only under their parent topics",withHarness(h=>{const state=h.api.getState();state.screen="packs";h.api.packs();for(const selector of ['[data-transit="transit"]','[data-transit="paratransit"]','[data-transit="sunline"]','[data-brain="known"]','[data-brain="riddles"]'])assert.equal(h.document.querySelector(selector),null,selector);h.click('[data-topic="transit"]');for(const id of ["transit","paratransit","sunline"])assert.ok(h.document.querySelector(`[data-transit="${id}"]`),id);h.click('[data-transit="paratransit"]');assert.deepEqual(Array.from(state.contentPacks),["paratransit"]);h.click('[data-topic="brain"]');for(const id of ["known","riddles"])assert.ok(h.document.querySelector(`[data-brain="${id}"]`),id);h.click('[data-brain="known"]');assert.deepEqual(Array.from(state.contentPacks),["paratransit","known"])}));
