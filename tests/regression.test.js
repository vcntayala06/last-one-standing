...
test("unfinished Game Setup and Who’s In sessions resume as setup data, never fake games",()=>{
 const setup=createHarness();try{const state=setup.api.getState();state.screen="setup";state.contentPacks=["original","work","transit"];setup.api.setup();setup.click('[data-setup-mode="original"]');setup.click('[data-setup-difficulty="hard"]');setup.click('[data-setup-seconds="20"]');setup.click('[data-setup-minutes="10"]');setup.click("#readQuestionsOff");setup.api.exitSetup?.();setup.speak("exit game");assert.equal(state.screen,"home");assert.ok(setup.document.querySelector("#resumeSaved"));setup.click("#resumeSaved");assert.equal(state.screen,"setup");assert.equal(state.game,null);assert.equal(state.mode,"original");assert.deepEqual(Array.from(state.contentPacks),["original","work","transit"]);assert.equal(state.difficulty,"hard");assert.equal(state.questionSeconds,20);assert.equal(state.duration,10);assert.equal(state.readQuestions,false)}finally{setup.close()}
...
test("Stage 6.15 production flow preserves every major screen in sequence",withHarness(h=>{
 const state=h.api.getState();state.mode="original";state.voiceOn=false;state.readQuestions=true;state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Blair"},{id:"p3",name:"Casey"}];state.screen="players";h.api.go("ready","production-playthrough");h.timers.advance(220);assert.equal(state.screen,"ready");h.click("#showtimeStart");assert.equal(state.screen,"transition");h.timers.advance(1600);assert.equal(state.screen,"handoff");h.timers.advance(3450);assert.equal(state.screen,"question");state.game.current={id:"flow-q",q:"What planet is red?",a:"Mars",cat:"Science"};state.game.answered=false;h.api.finish("correct");assert.equal(state.screen,"result");h.timers.advance(3200);assert.equal(state.screen,"transition");h.timers.advance(1600);assert.equal(state.screen,"handoff");state.game.players[2].eliminated=true;h.api.showdownIntro();assert.equal(state.screen,"showdown");h.timers.advance(3700);assert.equal(state.screen,"transition");h.timers.advance(1600);assert.equal(state.screen,"handoff");h.api.champion(state.game.players[0]);assert.equal(state.screen,"complete");const screens=h.window.__LOS_PLAYTEST_DIAGNOSTICS__.lifetimes().map(x=>x.screen);for(const expected of ["ready","handoff","transition","question","result","showdown","complete"])assert.ok(screens.includes(expected),expected)
}));
...
test("earned category knowledge and fast answers select one contextual host reaction",()=>{
 const street=createHarness();try{street.window.Math.random=()=>0;const state=setupQuestion(street),player=state.game.players[0],host=street.api.getHostSystem();state.game.current={id:"street-reaction",q:"Which artist recorded The Chronic?",a:"Dr. Dre",cat:"Music",packs:["street","music"]};state.game.questionStartedWith=15;state.game.questionRemaining=13;const before=host.history.length;street.api.finish("correct");assert.equal(host.history.length,before+1);assert.equal(host.history.at(-1).event,"streetKnowledge");assert.equal(host.history.at(-1).context.name,player.name);street.api.finish("correct");assert.equal(host.history.length,before+1)}finally{street.close()}
 const fast=createHarness();try{fast.window.Math.random=()=>0;const state=setupQuestion(fast),host=fast.api.getHostSystem();state.game.current={id:"fast-reaction",q:"What planet is red?",a:"Mars",cat:"Science & Nature",packs:["original"]};state.game.questionStartedWith=15;state.game.questionRemaining=14;const before=host.history.length;fast.api.finish("correct");assert.equal(host.history.length,before+1);assert.equal(host.history.at(-1).event,"fastCorrect")}finally{fast.close()}
});
