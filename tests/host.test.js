...
test("first, later, replay, and Solo turns each render exactly one Player-Up stage",async()=>{
 const provider=new FakeNaturalHostProvider(),h=createHarness({hostProvider:provider});try{
  ...
  h.api.startGame();assert.equal(state.screen,"handoff");assert.equal(history().filter(x=>x.to==="handoff").length,1);assert.equal(provider.calls.filter(x=>x.event==="firstTurn").length,1);
  h.api.advance();assert.equal(state.screen,"handoff");assert.equal(history().filter(x=>x.to==="handoff").length,2);assert.equal(provider.calls.filter(x=>x.event==="turn").length,1);
