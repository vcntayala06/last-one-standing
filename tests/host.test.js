...
test("first-player Host line protects the question clock and microphone recovery",async()=>{
 const provider=new FakeNaturalHostProvider(),h=createHarness({hostProvider:provider});try{const state=h.api.getState();state.mode="original";state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Blair"}];state.screen="ready";h.window.Math.random=()=>0;h.api.ready();provider.finish();await settle();assert.equal(state.screen,"transition");assert.equal(h.document.querySelector(".transition-big").textContent,"LOCK IN");assert.equal(h.document.querySelector(".lock-in-player").textContent,"Alex");assert.equal(provider.calls.at(-1).event,"lockIn");provider.finish();await settle();h.timers.advance(1600);assert.equal(state.screen,"handoff");assert.equal(provider.calls.at(-1).event,"firstTurn");assert.match(provider.calls.at(-1).text,/Alex/);assert.equal(h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().answerTimer,"stopped");provider.finish();await settle();h.timers.advance(449);assert.equal(h.document.getElementById("handoffCount").textContent,"");h.timers.advance(1);assert.equal(h.document.getElementById("handoffCount").textContent,"3");h.timers.advance(3000);assert.equal(state.screen,"question");assert.equal(provider.calls.at(-1).event,"questionRead");assert.equal(h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().questionReading,true);provider.finish();await settle();assert.ok(h.recognition()?.started);assert.equal(h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().answerTimer,"running");assert.equal(state.game.questionRemaining,state.questionSeconds)}finally{h.close()}
});

test("Solo Showtime uses a short Solo welcome and provider failure still starts safely",async()=>{
 const provider={available:true,calls:[],play(cue){this.calls.push(cue);return Promise.reject(new Error("offline"))},cancel(){},setVolume(){}},h=createHarness({hostProvider:provider});try{const state=h.api.getState();state.mode="solo";state.players=[{id:"p1",name:"Jordan"}];state.screen="ready";h.window.Math.random=()=>0;h.api.ready();assert.equal(provider.calls[0].event,"showtimeSolo");assert.match(provider.calls[0].text,/Jordan/);await settle();assert.equal(state.screen,"ready");h.click("#showtimeStart");assert.equal(state.screen,"transition");await settle();h.timers.advance(1600);await settle();h.timers.advance(1200);h.timers.advance(450);assert.equal(h.document.getElementById("handoffCount").textContent,"3");h.timers.advance(3000);await settle();assert.equal(state.screen,"question");assert.ok(state.game);assert.ok(provider.calls.some(x=>x.event==="soloTurn"));assert.ok(provider.calls.some(x=>x.event==="lockIn"));assert.ok(provider.calls.some(x=>x.event==="questionRead"));assert.equal(h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().answerTimer,"running") }finally{h.close()}
});

test("handoff countdown owns one 450ms beat and one 3-2-1 loop without starting the answer timer",()=>{
 const h=createHarness();try{const state=h.api.getState();state.mode="original";state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Blair"}];h.api.startGame();assert.equal(state.screen,"transition");h.timers.advance(1600);const count=()=>h.document.getElementById("handoffCount")?.textContent;assert.equal(count(),"");assert.equal(h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().answerTimer,"stopped");h.timers.advance(299);assert.equal(count(),"");h.timers.advance(150);assert.equal(count(),"");h.timers.advance(1);assert.equal(count(),"3");h.timers.advance(999);assert.equal(count(),"3");h.timers.advance(1);assert.equal(count(),"2");h.timers.advance(1000);assert.equal(count(),"1");h.timers.advance(999);assert.equal(state.screen,"handoff");h.timers.advance(1);assert.equal(state.screen,"question");assert.equal(h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().answerTimer,"running")}finally{h.close()}
});

test("leaving handoff cancels countdown and stale ticks cannot advance Home or a newer runtime",()=>{
 const h=createHarness();try{const state=h.api.getState();state.mode="original";state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Blair"}];h.api.startGame();h.timers.advance(1600);h.timers.advance(450);assert.equal(h.document.getElementById("handoffCount").textContent,"3");h.api.home();const session=h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().runtimeSessionId;h.timers.advance(30000);assert.equal(state.screen,"home");assert.equal(state.game!==null,true);assert.equal(h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().runtimeSessionId,session)}finally{h.close()}
});

test("Back and Exit cleanup invalidate an active handoff countdown",()=>{
 for(const action of ["back","leaveGame"]){const h=createHarness();try{const state=h.api.getState();state.mode="original";state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Blair"}];h.api.startGame();h.timers.advance(1600);h.timers.advance(450);assert.equal(h.document.getElementById("handoffCount").textContent,"3");h.api[action]();assert.equal(state.screen,"home",action);const session=h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().runtimeSessionId;h.timers.advance(30000);assert.equal(state.screen,"home",action);assert.equal(h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().runtimeSessionId,session,action)}finally{h.close()}}
});

test("handoff Host timeout cannot freeze the countdown",()=>{
 const provider=new FakeNaturalHostProvider(),h=createHarness({hostProvider:provider});try{const state=h.api.getState();state.mode="original";state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Blair"}];h.api.startGame();assert.equal(state.screen,"transition");h.timers.advance(1600);assert.equal(state.screen,"handoff");assert.equal(h.document.getElementById("handoffCount").textContent,"");h.timers.advance(19999);assert.equal(h.document.getElementById("handoffCount").textContent,"");h.timers.advance(1);assert.equal(h.document.getElementById("handoffCount").textContent,"");assert.ok(provider.cancels.includes("handoff-host-timeout"));h.timers.advance(449);assert.equal(h.document.getElementById("handoffCount").textContent,"");h.timers.advance(1);assert.equal(h.document.getElementById("handoffCount").textContent,"3");h.timers.advance(3000);assert.equal(state.screen,"question")}finally{h.close()}
});
...
test("first, later, replay, and Solo turns each render exactly one Player-Up stage",async()=>{
 const provider=new FakeNaturalHostProvider(),h=createHarness({hostProvider:provider});try{
  ...
  h.api.startGame();assert.equal(state.screen,"transition");h.timers.advance(1600);assert.equal(state.screen,"handoff");assert.equal(history().filter(x=>x.to==="handoff").length,1);assert.equal(provider.calls.filter(x=>x.event==="firstTurn").length,1);
  h.api.advance();assert.equal(state.screen,"transition");h.timers.advance(1600);assert.equal(state.screen,"handoff");assert.equal(history().filter(x=>x.to==="handoff").length,2);assert.equal(provider.calls.filter(x=>x.event==="turn").length,1);
  h.api.champion(state.game.players[0]);provider.finish();await settle();h.click("#playAgain");provider.finish();await settle();assert.equal(state.screen,"transition");h.timers.advance(1600);assert.equal(state.screen,"handoff");assert.equal(history().filter(x=>x.to==="handoff").length,3);
 }finally{h.close()}
 const solo=new FakeNaturalHostProvider(),s=createHarness({hostProvider:solo});try{const state=s.api.getState();state.mode="solo";state.players=[{id:"p1",name:"Jordan"}];s.api.startGame();assert.equal(state.screen,"transition");s.timers.advance(1600);assert.equal(state.screen,"handoff");assert.equal(s.window.__LOS_PLAYTEST_DIAGNOSTICS__.history().filter(x=>x.to==="handoff").length,1);assert.equal(solo.calls.filter(x=>x.event==="soloTurn").length,1);assert.equal(s.document.querySelectorAll(".handoff").length,1)}finally{s.close()}
});

test("Champion replay cancels obsolete Host speech and old completion cannot mutate the new match",async()=>{
 const provider=new FakeNaturalHostProvider(),h=createHarness({hostProvider:provider});try{const state=h.api.getState();state.mode="original";state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Blair"}];const staleRecognition=h.recognition();h.api.startGame();const oldGame=state.game;h.api.champion(oldGame.players[0]);assert.equal(provider.calls.at(-1).event,"champion");staleRecognition.emit("play again",{isFinal:true});assert.equal(state.screen,"complete");h.click("#playAgain");assert.equal(state.screen,"ready");assert.equal(state.game,null);assert.ok(provider.cancels.includes("runtime-change"));const session=h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().runtimeSessionId;provider.finish();await settle();assert.equal(state.screen,"transition");assert.notEqual(state.game,oldGame);assert.equal(h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().runtimeSessionId,session+1);h.timers.advance(1600);assert.equal(state.screen,"handoff")}finally{h.close()}
});
...
test("stale Host completion from an abandoned turn cannot advance Home",async()=>{
 const provider=new FakeNaturalHostProvider(),h=createHarness({hostProvider:provider});try{const state=h.api.getState();state.mode="original";state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Blair"}];h.window.Math.random=()=>0;h.api.startGame();assert.equal(state.screen,"transition");h.api.home();const session=h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().runtimeSessionId;provider.finish();await settle();h.timers.advance(10000);assert.equal(state.screen,"home");assert.equal(h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().runtimeSessionId,session)}finally{h.close()}
});
...
test("Final Showdown survives Host failure and stale callbacks for its full minimum sequence",async()=>{
 const provider={available:true,play:()=>Promise.reject(new Error("offline")),cancel(){},setVolume(){}},h=createHarness({hostProvider:provider});try{const state=h.api.getState();state.mode="original";state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Blair"},{id:"p3",name:"Casey"}];h.api.startGame();state.game.players[2].eliminated=true;h.api.showdownIntro();assert.equal(state.screen,"showdown");await settle();h.timers.advance(3500);assert.equal(state.screen,"showdown");h.timers.advance(200);assert.equal(state.screen,"transition");h.timers.advance(1600);assert.equal(state.screen,"handoff");h.api.home();h.timers.advance(30000);assert.equal(state.screen,"home")}finally{h.close()}
});

test("Stage 6.28 diagnostics correlate Host IDs transitions lifetimes and build identity",async()=>{
 const provider=new FakeNaturalHostProvider(),h=createHarness({hostProvider:provider});try{const state=h.api.getState();state.mode="original";state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Blair"}];state.screen="players";h.api.go("ready");const snap=h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot();assert.equal(snap.build.stage,"6.28");assert.equal(snap.build.version,"live-whos-in-voice-numbered-defaults-semantic-judge");assert.equal(snap.readQuestions,state.readQuestions);assert.equal(snap.voiceOn,state.voiceOn);assert.match(snap.hostEvent.hostEventId,/^host-/);assert.equal(provider.calls[0].hostEventId,snap.hostEvent.hostEventId);provider.finish();await settle();const lifetime=h.window.__LOS_PLAYTEST_DIAGNOSTICS__.lifetimes().find(x=>x.screen==="ready");assert.ok(lifetime.leftAt>=lifetime.enteredAt);assert.ok(lifetime.visibleDurationMs>=0)}finally{h.close()}
});
