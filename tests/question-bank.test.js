...
const seed=require("../question-bank-data"),batch=require("../question-bank-batch-1"),batch2=require("../question-bank-batch-2"),batch3=require("../question-bank-batch-3"),batch4=require("../question-bank-batch-4"),batch5=require("../question-bank-batch-5"),batch6=require("../question-bank-batch-6"),batch7=require("../question-bank-batch-7"),api=require("../question-bank"),data={...seed,bankVersion:"stage-6.32-major-music-depth",questions:[...seed.questions,...batch.questions,...batch2.questions,...batch3.questions,...batch4.questions,...batch5.questions,...batch6.questions,...batch7.questions]};
const clone=value=>JSON.parse(JSON.stringify(value));

test("all seeds and population batches pass the versioned schema with stable unique IDs",()=>{
 const report=api.validateBank(data,{nearDuplicates:true});assert.equal(report.valid,true,report.errors.join("\n"));assert.equal(seed.questions.length,34);assert.equal(batch.questions.length,500);assert.equal(batch2.questions.length,550);assert.equal(batch3.questions.length,550);assert.equal(batch4.questions.length,18);assert.equal(batch5.questions.length,22);assert.equal(batch6.questions.length,240);assert.equal(batch7.questions.length,348);assert.equal(report.count,2262);assert.equal(new Set(data.questions.map(q=>q.id)).size,2262);assert.equal(data.questions.every(q=>q.schemaVersion===1&&q.revision>=1),true);assert.equal(report.warnings.filter(x=>x.includes("near-duplicate")).length,0)
});

test("Build 6.32 audits every playable question and passes the strengthened wording gate",()=>{const bank=api.createQuestionBank(data),audit=bank.audit();assert.equal(audit.audited,2130);assert.equal(audit.passed,2130);assert.deepEqual(audit.issues,[]);assert.equal(bank.questions.some(q=>/movie or TV show|TV show or movie/i.test(q.prompt)),false)});
...
test("Build 6.32 Music selection exhausts without repetition while Disney 6.31 remains intact",()=>{const bank=api.createQuestionBank(data),used=[];for(;;){const q=bank.select({packs:["music"],difficulty:"medium",usedIds:used,random:()=>0});if(!q)break;assert.equal(used.includes(q.id),false,q.id);used.push(q.id)}assert.equal(new Set(used).size,used.length);assert.equal(batch6.questions.length,240);assert.equal(bank.questions.filter(q=>bank.packsFor(q).includes("disney")).length,245)});

test("Build 6.31 adds a substantial, balanced, filterable Disney library",()=>{const bank=api.createQuestionBank(data),disney=bank.questions.filter(q=>bank.packsFor(q).includes("disney")),fresh=batch6.questions,count=tag=>fresh.filter(q=>q.disney.tags.includes(tag)).length;assert.equal(disney.length,245);assert.equal(fresh.length,240);assert.ok(count("descendants")>=25);assert.ok(count("zombies")>=25);assert.ok(count("camp-rock")>=15);assert.ok(count("wizards")>=20);assert.ok(count("vampirina")>=15);assert.ok(count("fairy-tales")>=60);assert.ok(count("disneyland")>=50);assert.ok(fresh.flatMap(q=>q.disney.tags).every(tag=>api.DISNEY_METADATA_TAGS.includes(tag)))});
...
test("duplicate canonical answers are reported for review but not silently deleted",()=>{
 const suspicious=clone(data);suspicious.questions[1].answer.canonical=suspicious.questions[0].answer.canonical;const report=api.validateBank(suspicious);assert.equal(report.valid,true);assert.ok(report.warnings.some(x=>x.includes("Suspicious repeated canonical answer")));assert.equal(report.count,2262)
});

test("edition indexes make Original, Work, and Solo eligibility explicit",()=>{
 const bank=api.createQuestionBank(data);assert.equal(bank.byEdition.get("original").length,2037);assert.equal(bank.byEdition.get("work").length,2121);assert.equal(bank.byEdition.get("solo").length,2037);assert.ok(bank.byEdition.get("work").every(q=>q.editions.includes("work")&&q.workSafe));assert.ok(bank.byEdition.get("solo").every(q=>q.editions.includes("solo")))
});
