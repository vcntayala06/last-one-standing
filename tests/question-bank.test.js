...
const seed=require("../question-bank-data"),batch=require("../question-bank-batch-1"),batch2=require("../question-bank-batch-2"),batch3=require("../question-bank-batch-3"),batch4=require("../question-bank-batch-4"),batch5=require("../question-bank-batch-5"),batch6=require("../question-bank-batch-6"),batch7=require("../question-bank-batch-7"),batch8=require("../question-bank-batch-8"),batch9=require("../question-bank-batch-9"),batch10=require("../question-bank-batch-10"),api=require("../question-bank"),data={...seed,bankVersion:"stage-6.38-current-generation-music-expansion",questions:[...seed.questions,...batch.questions,...batch2.questions,...batch3.questions,...batch4.questions,...batch5.questions,...batch6.questions,...batch7.questions,...batch8.questions,...batch9.questions,...batch10.questions]};
const clone=value=>JSON.parse(JSON.stringify(value));

test("all seeds and population batches pass the versioned schema with stable unique IDs",()=>{
 const report=api.validateBank(data,{nearDuplicates:true});assert.equal(report.valid,true,report.errors.join("\n"));assert.equal(seed.questions.length,34);assert.equal(batch.questions.length,500);assert.equal(batch2.questions.length,550);assert.equal(batch3.questions.length,550);assert.equal(batch4.questions.length,18);assert.equal(batch5.questions.length,22);assert.equal(batch6.questions.length,240);assert.equal(batch7.questions.length,348);assert.equal(batch8.questions.length,90);assert.equal(batch9.questions.length,176);assert.equal(batch10.questions.length,191);assert.equal(report.count,2719);assert.equal(new Set(data.questions.map(q=>q.id)).size,2719);assert.equal(data.questions.every(q=>q.schemaVersion===1&&q.revision>=1),true);assert.equal(report.warnings.filter(x=>x.includes("near-duplicate")).length,0)
});
...
test("Build 6.35 audits every playable question and passes the strengthened wording gate",()=>{const bank=api.createQuestionBank(data),audit=bank.audit();assert.equal(audit.audited,2529);assert.equal(audit.passed,2529);assert.deepEqual(audit.issues,[]);assert.equal(bank.questions.some(q=>/movie or TV show|TV show or movie|describes tool whose|historical event or document (?:religious movement|labor organization) associated/i.test(q.prompt)),false)});
...
test("Build 6.32 Music selection remains intact without repetition",()=>{const bank=api.createQuestionBank(data),used=[];for(;;){const q=bank.select({packs:["music"],difficulty:"medium",usedIds:used,random:()=>0});if(!q)break;assert.equal(used.includes(q.id),false,q.id);used.push(q.id)}assert.equal(new Set(used).size,used.length);assert.equal(batch7.questions.length,348);assert.equal(bank.questions.filter(q=>q.subject==="Music").length,746)});

test("Build 6.38 adds 191 approved current-generation Music questions",()=>{assert.equal(batch10.candidateCount,192);assert.equal(batch10.removedDuringAudit,2);assert.equal(batch10.yukonAdded,1);assert.equal(batch10.questions.length,191);for(const q of batch10.questions){assert.equal(q.subject,"Music",q.id);assert.equal(q.review.status,"approved",q.id);assert.ok(["easy","medium","hard"].includes(q.difficulty),q.id);assert.ok(q.answer.canonical&&q.answer.accepted.en.length,q.id);assert.ok(q.music.eras.includes("today"),q.id);assert.equal(q.fact.dateSensitive,false,q.id);assert.equal(q.fact.sources.length,2,q.id);assert.equal(new Set(q.fact.sources.map(s=>s.publisher+"|"+s.url)).size,2,q.id);assert.ok(q.fact.sources.every(s=>s.confirmsAnswer&&s.confirmsWording),q.id)}});

test("Build 6.38 modern sample and natural answer variants remain intact",()=>{const find=answer=>batch10.questions.find(q=>q.answer.canonical===answer);for(const answer of ["Ariana Grande","Peso Pluma","YG","Kendrick Lamar","Billie Eilish","Karol G","SZA","Shaboozey"])assert.ok(find(answer),answer);assert.ok(find("Beyoncé").answer.accepted.en.includes("beyonce"));assert.ok(find("Carín León").answer.accepted.en.includes("carin leon"));assert.ok(find("Måneskin").answer.accepted.en.includes("maneskin"));assert.ok(batch10.questions.some(q=>q.prompt.includes("yes, and?")&&q.answer.canonical==="Ariana Grande"));assert.ok(batch10.questions.some(q=>q.prompt.includes("La Patrulla")&&q.answer.canonical==="Peso Pluma"));assert.ok(batch10.questions.some(q=>q.prompt.includes("Toxic")&&q.answer.canonical==="YG"));const yukon=batch10.questions.find(q=>q.prompt==="Who performs the song “YUKON”?");assert.equal(yukon.answer.canonical,"Justin Bieber");assert.ok(yukon.answer.accepted.en.includes("bieber"))});

test("Build 6.38 has no exact or normalized duplicate prompts",()=>{const normalize=s=>s.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/g," ").trim(),all=data.questions.map(q=>q.prompt),exact=new Set(),normalized=new Set();for(const prompt of all){assert.equal(exact.has(prompt),false,`exact:${prompt}`);exact.add(prompt);const key=normalize(prompt);assert.equal(normalized.has(key),false,`normalized:${prompt}`);normalized.add(key)}});
...
test("duplicate canonical answers are reported for review but not silently deleted",()=>{
 const suspicious=clone(data);suspicious.questions[1].answer.canonical=suspicious.questions[0].answer.canonical;const report=api.validateBank(suspicious);assert.equal(report.valid,true);assert.ok(report.warnings.some(x=>x.includes("Suspicious repeated canonical answer")));assert.equal(report.count,2719)
});

test("edition indexes make Original, Work, and Solo eligibility explicit",()=>{
 const bank=api.createQuestionBank(data);assert.equal(bank.byEdition.get("original").length,2444);assert.equal(bank.byEdition.get("work").length,2520);assert.equal(bank.byEdition.get("solo").length,2444);assert.ok(bank.byEdition.get("work").every(q=>q.editions.includes("work")&&q.workSafe));assert.ok(bank.byEdition.get("solo").every(q=>q.editions.includes("solo")))
});
