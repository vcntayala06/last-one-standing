...
test("Build 6.34 audits every playable question and passes the strengthened wording gate",()=>{const bank=api.createQuestionBank(data),audit=bank.audit();assert.equal(audit.audited,2340);assert.equal(audit.passed,2340);assert.deepEqual(audit.issues,[]);assert.equal(bank.questions.some(q=>/movie or TV show|TV show or movie/i.test(q.prompt)),false)});
...
test("edition indexes make Original, Work, and Solo eligibility explicit",()=>{
 const bank=api.createQuestionBank(data);assert.equal(bank.byEdition.get("original").length,2255);assert.equal(bank.byEdition.get("work").length,2331);assert.equal(bank.byEdition.get("solo").length,2255);assert.ok(bank.byEdition.get("work").every(q=>q.editions.includes("work")&&q.workSafe));assert.ok(bank.byEdition.get("solo").every(q=>q.editions.includes("solo")))
});
