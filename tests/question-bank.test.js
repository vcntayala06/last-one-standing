...
const seed=require("../question-bank-data"),batch=require("../question-bank-batch-1"),batch2=require("../question-bank-batch-2"),batch3=require("../question-bank-batch-3"),batch4=require("../question-bank-batch-4"),batch5=require("../question-bank-batch-5"),batch6=require("../question-bank-batch-6"),api=require("../question-bank"),data={...seed,bankVersion:"stage-6.31-disney-expansion",questions:[...seed.questions,...batch.questions,...batch2.questions,...batch3.questions,...batch4.questions,...batch5.questions,...batch6.questions]};
const clone=value=>JSON.parse(JSON.stringify(value));

test("all seeds and population batches pass the versioned schema with stable unique IDs",()=>{
 const report=api.validateBank(data,{nearDuplicates:true});assert.equal(report.valid,true,report.errors.join("\n"));assert.equal(seed.questions.length,34);assert.equal(batch.questions.length,500);assert.equal(batch2.questions.length,550);assert.equal(batch3.questions.length,550);assert.equal(batch4.questions.length,18);assert.equal(batch5.questions.length,22);assert.equal(batch6.questions.length,240);assert.equal(report.count,1914);assert.equal(new Set(data.questions.map(q=>q.id)).size,1914);assert.equal(data.questions.every(q=>q.schemaVersion===1&&q.revision>=1),true);assert.equal(report.warnings.filter(x=>x.includes("near-duplicate")).length,0)
});

test("Build 6.31 audits every playable question and passes the strengthened wording gate",()=>{const bank=api.createQuestionBank(data),audit=bank.audit();assert.equal(audit.audited,1782);assert.equal(audit.passed,1782);assert.deepEqual(audit.issues,[])});
...
test("duplicate canonical answers are reported for review but not silently deleted",()=>{
 const suspicious=clone(data);suspicious.questions[1].answer.canonical=suspicious.questions[0].answer.canonical;const report=api.validateBank(suspicious);assert.equal(report.valid,true);assert.ok(report.warnings.some(x=>x.includes("Suspicious repeated canonical answer")));assert.equal(report.count,1914)
});

test("edition indexes make Original, Work, and Solo eligibility explicit",()=>{
 const bank=api.createQuestionBank(data);assert.equal(bank.byEdition.get("original").length,1689);assert.equal(bank.byEdition.get("work").length,1773);assert.equal(bank.byEdition.get("solo").length,1689);assert.ok(bank.byEdition.get("work").every(q=>q.editions.includes("work")&&q.workSafe));assert.ok(bank.byEdition.get("solo").every(q=>q.editions.includes("solo")))
});
